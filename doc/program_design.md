...
MVP は、1 ステージ、固定長オブジェクト管理、60fps、480x640 論理画面で実装する。表示倍率は 1 倍または 2 倍（960x1280）とし、ゲームロジックは常に論理座標で処理する。

データ定義と実行時状態を分離する。詳細なデータフォーマット仕様は [doc/data_formats.md](doc/data_formats.md) を参照。
...
ゲームロジック、ウィンドウイベント、GPU描画、入力、音声は接続済みである。抽象traitはテストと将来の差し替えのために残している。
...
| `gilrs` | `0.11` | ゲームパッドのボタン、十字キー、スティック、接続イベント | ゲーム状態、当たり判定、描画 |
...
| `game/update.rs` | 60fps の 1 フレーム更新順序 |
| `game/movement.rs` | 32 方向、円軌道、ベジェ曲線、背景同期移動 |
...
キャラクタ画像の読み込みと `wgpu` テクスチャへの変換は実装済みである。`AssetCatalog` がステージ画像を読み込み、`GpuRenderer` がプレイヤー、敵、ボス、アイテム、自弾、敵弾、背景をテクスチャ描画する。今後の拡張候補は本番素材への差し替え、ステージ単位のアセット切り替え、設定画面である。
...
struct PlayerState {
    ...
    growth_level: u8,
    recovery_stock: u8,
    player_group_fire_cooldown: u16, // 60fpsで30フレーム=0.5秒
}
```
...
const PLAYER_BULLET_CAPACITY: usize = 1024;
const ENEMY_BULLET_CAPACITY: usize = 128;
```

プールは `active` が false のスロットを先頭から検索する。空きがない場合は新規生成を破棄し、ゲームループは継続する。

自弾は同時発射した弾にグループIDを付ける。アクティブなグループの弾が1発でも残っている間は次のグループを発射せず、グループ内の全弾が消滅した後に次の入力を発射対象とする。リボーン時には敵、敵弾、自弾、アイテムのプールをクリアする。プレイヤーの成長段階と回復ストックは保持する。
...
32 方向を速度ベクトルへ変換し、Q12.4 のまま位置へ加算する。
...
敵は背景スクロール量に同期して位置を更新する。軌道適用中は画面外でも削除されないが、軌道終了後は背景スクロールとともに移動し、画像矩形全体が画面外へ出た時点で削除（プール解放）する。
...
- 敵は軌道適用中なら画面外でも削除しない
- 敵の軌道終了後は背景スクロールに同期して移動し、画像矩形全体が画面外へ出た時点で削除する
