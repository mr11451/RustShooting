...
- 論理画面サイズは `640x320`
- 表示サイズは `640x320` またはその 2 倍の `1280x640`
- ステージデータの座標は常に論理画面 `640x320` を基準とし、表示倍率では変更しない
- 処理速度は `30fps`
...
- 座標の原点は画面左上
- `x` は右方向、`y` は下方向を正とする
- 画面内の論理座標は `x: 0..=639`、`y: 0..=319` を基本とする
...
- 自弾と敵弾の画像矩形を中心座標 `(x, y)`、半幅 `half_width`、半高 `half_height` で表す場合、`x + half_width < 0`、`x - half_width >= 640`、`y + half_height < 0`、`y - half_height >= 320` のいずれかを満たした時点で削除する
- 画面外へ出た成長アイテムも削除する
- 敵は軌道データの適用中であれば画面外へ出ても削除しない
- 敵の軌道データが終了した後は、敵の位置更新を背景スクロール量に同期する
...
ゲーム画面の HUD とデモ画面のランキングを定義する。座標は論理画面 `640x320` 基準で、表示倍率には依存しない。
...
| 項目 | 型の候補 | 初期位置・サイズ | 内容 |
|---|---|---|---|
| `hp_gauge` | `HudElement` | `x=8, y=304, width=160, height=8` | 現在 HP と最大 HP の割合を表示 |
| `score` | `HudElement` | `x=176, y=304, width=160, height=8` | 得点値を表示 |
| `lives` | `HudElement` | `x=344, y=304, width=96, height=8` | 残自キャラ数を表示 |
| `stock` | `HudElement` | `x=448, y=304, width=184, height=8` | 成長または HP 回復ストックを表示 |
